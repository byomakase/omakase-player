import {MarkerLane, MomentMarker, OmakasePlayer, PeriodMarker, SubtitlesLane, ThumbnailLane} from "../src";
import {RandomUtil} from "../src/util/random-util";
import {ColorUtil} from "../src/util/color-util";
import {forkJoin} from "rxjs";
import Hls from "hls.js";
import {AudioTrackLane} from "../src";
...
window.addEventListener('load', () => {
    ...
    createSelectOptions(testStreams);

    let testStreamToLoad = 0;
    ...
    omakasePlayer.loadVideo(testStreams[testStreamToLoad].video, testStreams[testStreamToLoad].fps).subscribe({
        next: (video) => {
            ...
            let daDkSubtitles$ = omakasePlayer.subtitles.createVttTrack({
                id: 'da-dk-1', src: testStreams[testStreamToLoad].dkSubtitle, label: 'Dutch', language: 'da-DK', default: true
            })

            let enUsSubtitles$ = omakasePlayer.subtitles.createVttTrack({
                id: 'en-us-1', src: testStreams[testStreamToLoad].enCaption, label: 'English', language: 'en-US'
            })
            ...
            forkJoin([daDkSubtitles$, enUsSubtitles$]).subscribe(([daDkSubtitles, enUsSubtitles]) => {
    ...
    omakasePlayer.createTimeline({
        thumbnailVttUrl: testStreams[testStreamToLoad].thumbnails
    }).subscribe((timeline) => {
    ...
    let createTimelineLanes = (timeline) => {
        let defaultThumbnailLane = timeline.getLane('thumbnail_lane_default');
        ...
        let subtitlesLane1 = timeline.getLane('subtitles_lane_1');
        let subtitlesLane2 = timeline.getLane('subtitles_lane_2');
        let audioTrackLane1 = timeline.getLane('audio_track_lane_1');
        let audioTrackLane2 = timeline.getLane('audio_track_lane_2');
        ...
        if (!defaultThumbnailLane) {
            let defaultThumbnailLane = new ThumbnailLane({
        ...
        if (!subtitlesLane1) {
            subtitlesLane1 = new SubtitlesLane({
                id: "subtitles_lane_1",
                description: "Danish subtitle",
                subtitlesVttUrl: testStreams[testStreamToLoad].dkSubtitle,
                style: {
        ...
        if (!subtitlesLane2) {
            subtitlesLane2 = new SubtitlesLane({
                id: "subtitles_lane_2",
                description: "English caption",
                subtitlesVttUrl: testStreams[testStreamToLoad].enCaption,
                style: {
                    backgroundFill: "#eaeaea",
                    paddingTop: 5,
                    paddingBottom: 5
        ...
        if (!audioTrackLane1) {
            audioTrackLane1 = new AudioTrackLane({
                id: 'audio_track_lane_1',
                description: '2.0 audio track',
                audioVttFileUrl: testStreams[testStreamToLoad].audioLvl20,
                style: {
        ...
        if (!audioTrackLane2) {
            audioTrackLane2 = new AudioTrackLane({
                id: 'audio_track_lane_2',
                description: '5.1 audio track',
                audioVttFileUrl: testStreams[testStreamToLoad].audioLvl51,
                style: {
                    backgroundFill: "#ffffff",
                    paddingTop: 5,
                    paddingBottom: 5,
        ...
        let markerLanes = [inAndOutMarkersLane, inAndOutMarkersLane2];
    ...
    streamsSelect.onchange = (event) => {
        let index = streamsSelect.value;

        let video = testStreams[index];
        let videoUrl = video.url;
        let videoFps = video.fps;
