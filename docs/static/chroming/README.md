...
#### `setFloatingTimeVisible(visible: boolean, timeInteractive?: boolean, openEditMode?: boolean): Observable<void>`

Shows or hides the floating time display. Optionally updates whether the time display is interactive (seekable by double-clicking) and whether to immediately open it in edit mode.

```javascript
// Show the floating time display
omakasePlayer.chroming.setFloatingTimeVisible(true).subscribe(() => {
  console.log('Floating time is now visible');
});

// Show and make the time display interactive (seekable by double-clicking)
omakasePlayer.chroming.setFloatingTimeVisible(true, true).subscribe(() => {
  console.log('Floating time is now visible and interactive');
});

// Show, make interactive, and immediately open in edit mode
omakasePlayer.chroming.setFloatingTimeVisible(true, true, true).subscribe(() => {
  console.log('Floating time is visible and in edit mode');
});
```
