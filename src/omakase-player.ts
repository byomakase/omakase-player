...
export class OmakasePlayer implements OmakasePlayerApi, Destroyable {
  ...
  loadVideo(videoSourceUrl: string, videoFrameRate: number, options?: VideoLoadOptions): Observable<Video> {
    return new Observable<Video>(o$ => {
      this._videoController.loadVideo(videoSourceUrl, videoFrameRate, options).pipe(first()).subscribe({
        next: video => {
          o$.next(video);
          o$.complete();
        },
  ...
  createTimeline(config: Partial<ConfigWithOptionalStyle<TimelineConfig>>): Observable<TimelineApi> {
    return new Observable<Timeline>(o$ => {
      ...
      let videoReady$ = new Subject<void>();
      let yogaLayoutReady$ = new Subject<void>();

      forkJoin([videoReady$, yogaLayoutReady$]).pipe(takeUntil(this._destroyed$)).subscribe(() => {
        createTimeline();
      ...
      YogaProvider.instance().init().pipe(takeUntil(this._destroyed$)).subscribe(() => {
        ...
        // console.debug('yoga-layout ready')
      })
      ...
      if (!this.timeline) {
        ...
        if (this._videoController.isVideoLoaded()) {
          ...
          this._videoController.onVideoLoaded$.pipe(filter(p => !!p), first()).subscribe(video => {
