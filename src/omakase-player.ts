...
import {HTMLVideoElementCrossorigin, VideoController} from "./video/video-controller";
...
export interface OmakasePlayerConfig {
    ...
    crossorigin: HTMLVideoElementCrossorigin,
...
export class OmakasePlayer implements OmakasePlayerApi, Destroyable {
    ...
    constructor(config: WithOptionalPartial<OmakasePlayerConfig, 'style'>) {
        ...
        this.playerHTMLElementId = this.config.playerHTMLElementId;

        this.domController = new DomController(this.playerHTMLElementId);
        this.videoController = new VideoHlsController(this.domController, this.config.crossorigin);
        ...
        this.domController.videoController = this.videoController;
    ...
    createTimeline(config: Partial<ComponentConfigStyleComposed<TimelineConfig>>): Observable<Timeline> {
        return new Observable<Timeline>(o$ => {
            let createTimeline = () => {
                ...
                this.domController.timeline = this.timeline;
    ...
    destroy() {
        let destroyables: Destroyable[] = [this._timeline, this.videoController, this.domController, this.audioController, this.subtitlesController]
