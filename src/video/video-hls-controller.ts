...
import Hls, {ErrorData, MediaPlaylist} from "hls.js";

import {first, forkJoin, fromEvent, Observable, takeUntil} from "rxjs";
...
export class VideoHlsController extends VideoController {
    ...
    videoLoad(sourceUrl: string, frameRate: number, duration: number): Observable<Video> {
        return new Observable<Video>(o$ => {
            ...
            forkJoin([hlsMediaAttached$, hlsManifestParsed$, videoLoadedData$]).pipe(first()).subscribe(result => {
