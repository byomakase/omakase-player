...
export abstract class VideoController implements VideoApi, Destroyable {
    protected browserProvider: BrowserProvider = BrowserProvider.instance();
            ...
            if (!isNaN(newTime)) {
                ...
                fromEvent(this.videoElement, HTMLVideoElementEventKeys.SEEKED).pipe(takeUntil(this.seekBreaker$), take(1)).subscribe((event) => {
                    this.syncVideoFrames(syncConditions).subscribe(result => {
                        ...
                        if (this.getCorrectedDuration() > this.getHTMLVideoElement().duration) {
                            /**
                             * If we land on exact time of the frame start at the end of video, there is the chance that we won't load the frame
                             */
                            this.video.setCorrectedDuration(Number.isInteger(this.getHTMLVideoElement().duration * this.video.frameRate) ? this.getHTMLVideoElement().duration - this.frameDurationSpillOverCorrection : this.getHTMLVideoElement().duration);
                        }
    ...
    calculateFrameToTime(framesCount: number): number {
        return FrameUtil.frameToTime(framesCount, this.getFrameRateDecimal());
    }
    ...
    formatTimestamp(time: number): string {
        ...
        return TimeUtil.formatVideoTimestamp(time, this.getFrameRateDecimal());
    }
    ...
    isFullscreen(): boolean {
        ...
        return this.domController.isFullscreen();
    }
    ...
    toggleFullscreen() {
        ...
        this.domController.toggleFullscreen(this.videoElement);
    }
