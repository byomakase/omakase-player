...
export class ObservationTrackDownsampler extends TimedItemsTrackDownsampler<ObservationTrack<ObservationTrackState, ObservationTrackEvent>> {
  constructor(sourceTrack: ObservationTrack, options: DownsampleOptions) {
    super(sourceTrack as ObservationTrack<ObservationTrackState, ObservationTrackEvent>, options);
  }
  ...
  protected resolveDownsampledTimedItem(
    _index: number,
    start: number,
    end: number,
    ...
    return new DefaultObservation({
      ...
      label: 'DOWNSAMPLED',
      items,
      data: {sourceItemIds: timedItems.map((item) => item.id)},
    });
  }

  private _getAggregateValue(values: number[]): number {
    switch (this._options.downsampleStrategy) {
      case 'max':
      ...
      case 'none':
        return values[0]!;
      default:
        throw new Error('Unsupported downsampling strategy: ' + this._options.downsampleStrategy);
    }
  }
}
