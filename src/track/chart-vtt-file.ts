...
import {BarChartVttCue, ChartVttCue, LineChartVttCue, OgChartVttCue} from '../types';
import {map, Observable} from 'rxjs';
import {z} from 'zod';
import Decimal from 'decimal.js';
import {AxiosRequestConfig} from 'axios';

export class ChartVttFile extends BaseOmakaseRemoteVttFile<ChartVttCue> {

  protected constructor(url: string, axiosConfig?: AxiosRequestConfig) {
    super(url, axiosConfig);
  }

  static create(url: string, axiosConfig?: AxiosRequestConfig): Observable<ChartVttFile> {
    let instance = new ChartVttFile(url, axiosConfig);
    return instance.fetch().pipe(map(result => {
      return instance;
    }))
  }

  protected mapCue(vttCueParsed: VttCueParsed, index: number): ChartVttCue {
    return {
      index: index,
...
export class LineChartVttFile extends BaseOmakaseRemoteVttFile<LineChartVttCue> {
  ...
  protected mapCue(vttCueParsed: VttCueParsed, index: number): LineChartVttCue {
    return {
      index: index,
      id: vttCueParsed.identifier,
      startTime: new Decimal(vttCueParsed.start).toDecimalPlaces(3).toNumber(),
...
export class BarChartVttFile extends BaseOmakaseRemoteVttFile<BarChartVttCue> {
  ...
  protected mapCue(vttCueParsed: VttCueParsed, index: number): BarChartVttCue {
    return {
      index: index,
      id: vttCueParsed.identifier,
      startTime: new Decimal(vttCueParsed.start).toDecimalPlaces(3).toNumber(),
...
export class OgChartVttFile extends BaseOmakaseRemoteVttFile<OgChartVttCue> {
  ...
  protected mapCue(vttCueParsed: VttCueParsed, index: number): OgChartVttCue {
    return {
      index: index,
      id: vttCueParsed.identifier,
      startTime: new Decimal(vttCueParsed.start).toDecimalPlaces(3).toNumber(),
