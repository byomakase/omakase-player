...
import {catchError, debounceTime, forkJoin, map, Observable, of, Subject, switchMap, take, takeUntil} from "rxjs";
...
export class ThumbnailLane extends BaseTimelineLane<ThumbnailLaneConfig, ThumbnailLaneStyle> {
    ...
    private fetchThumbnailVttFile(url: string, axiosConfig: AxiosRequestConfig, thumbnailHeight: number): Observable<{thumbnailVttFile: ThumbnailVttFile, thumbnailWidth: number}> {
        if (url) {
