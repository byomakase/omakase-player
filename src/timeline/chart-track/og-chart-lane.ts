...
import {OgChart, OgChartCue, WithOptionalPartial} from '../../types';
...
const styleDefault: OgChartLaneStyle = {
...
export class OgChartLane extends BaseTimelineLane<OgChartLaneConfig, OgChartLaneStyle> {
  // region config
  ...
  // region components
  protected readonly chartLaneItemsMap: Map<number, OgChartLaneItem> = new Map<number, OgChartLaneItem>();
  // endregion
  ...
  private getVisibleCuesForInterpolation(): OgChartCue[] {
  ...
  private resolveCuesInterpolations(): Map<number, OgChartCue> {
    ...
    for (let i = 0; i < this.numOfInterpolations; i++) {
      ...
      if (cuesForInterpolation.length > 0) {
        ...
        // TODO here we can interpolate values in between
        cue = {
          ...cuePartial,
          value: 0,
        }
