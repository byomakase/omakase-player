...
export interface ScrollableHorizontally {
  scrollHorizontally(percent: number): void;
...
export class Scrollbar extends BaseKonvaComponent<ScrollbarConfig, ScrollbarStyle, Konva.Group> implements OnMeasurementsChange, HasRectMeasurement {
  ...
  constructor(config: Partial<ConfigWithOptionalStyle<ScrollbarConfig>>) {
    ...
    this._handleBar = new Konva.Rect({
      ...Constants.POSITION_TOP_LEFT,
    ...
    this._styleAdapter.onChange$.pipe(takeUntil(this._destroyed$), filter(p => !!p)).subscribe((style) => {
      this.onStyleChange();
    })
  ...
  getScrollHandlePercent(): number {
    ...
    return scrollHandleX === 0 ? 0 : new Decimal(this._handleBar.x()).div(this._group.width() - this._handleBar.width()).mul(100).toNumber();
  }
