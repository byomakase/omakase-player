...
export interface ZoomScrollbarStyle {
  height: number;
...
export interface ZoomScrollbarConfig {
  x: number;
  y: number;
  width: number;
  zoomMax: number;
...
const configDefault: ZoomScrollbarConfig = {
  x: 0,
  y: 0,
  width: 0,
  zoomMax: 2000,
...
export const ZOOM_SCROLLBAR_STYLE_DEFAULT: ZoomScrollbarStyle = {
  height: 20,
...
export enum ZoomScrollbarEventType {
  SCROLLBAR_SCROLL = 'SCROLLBAR_SCROLL',
  SCROLLBAR_ZOOM = 'SCROLLBAR_ZOOM',
}

export type ZoomScrollbarEventTypeDataMap = {
  [ZoomScrollbarEventType.SCROLLBAR_SCROLL]: {
    scrollPercent: number;
  };
  [ZoomScrollbarEventType.SCROLLBAR_ZOOM]: {
    zoomPercent: number;
    zoomFocus: number;
  };
};

export type ZoomScrollbarEvent = {
  [K in ZoomScrollbarEventType]: {
    type: K;
    data: ZoomScrollbarEventTypeDataMap[K];
  };
}[keyof ZoomScrollbarEventTypeDataMap];

export class ZoomScrollbar extends BaseKonvaComponent2<Konva.Group> implements OnMeasurementsChange, HasRectMeasurement {
  private readonly _onEvent$: Subject<ZoomScrollbarEvent> = new Subject<ZoomScrollbarEvent>();

  protected _ui: Ui;

  protected _config: ZoomScrollbarConfig;
  protected _providedStyle?: Partial<ZoomScrollbarStyle> | undefined;
  protected _styledElement: StyledElementWithId<ZoomScrollbarStyle>;
  protected _style: ZoomScrollbarStyle;
  protected _initialStyle?: ZoomScrollbarStyle;
  ...
  constructor(ompProvider: OmpProvider, configAndStyle?: ConfigAndStyle<ZoomScrollbarConfig, ZoomScrollbarStyle>) {
    super();
    ...
    this._styledElement = {
      id: CryptoUtil.uuid(),
      // Keep the public style-class key as 'Scrollbar' — ElementStyleByName['Scrollbar'] is part
      // of the public styling API (src/ui/styles.ts), independent of this class's internal name.
    ...
    this._style = this._ui.resolveStyle<ZoomScrollbarStyle>(this._styledElement) as ZoomScrollbarStyle;
    this._initialStyle = {
      ...this._style,
    };
    ...
    this._handleBar.on('dragstart dragmove dragend', (event) => {
      ...
      if (rpp) {
        if (rpp.x >= 0 && rpp.x <= this._handleBar.width()) {
          this._onEvent$.next({
            type: ZoomScrollbarEventType.SCROLLBAR_SCROLL,
            data: {
    ...
    this._leftZoomHandle.on('dragstart dragmove dragend', (event) => {
      ...
      this._onEvent$.next({
        type: ZoomScrollbarEventType.SCROLLBAR_ZOOM,
        data: {
  ...
  get onEvent$(): Observable<ZoomScrollbarEvent> {
  ...
  protected handleStyleUpdate(): void {
    this._style = this._ui!.resolveStyle(this._styledElement) as ZoomScrollbarStyle;
  ...
  setStyle(style: Partial<ZoomScrollbarStyle>) {
