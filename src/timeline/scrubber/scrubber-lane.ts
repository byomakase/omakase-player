...
import {ScrubberLaneApi} from '../../api/scrubber-lane-api';
...
export class ScrubberLane extends BaseTimelineLane<ScrubberLaneConfig, ScrubberLaneStyle> implements ScrubberLaneApi {
  ...
  private _timecodeDisplay?: TimecodeDisplay;
  ...
  override prepareForTimeline(timeline: Timeline, videoController: VideoControllerApi) {
    ...
    this._ticksGroup = new Konva.Group({
    ...
    this._timecodedEventCatcher.on('mousemove', (event) => {
      this.onMouseMove$.next({
        evt: event.evt,
        position: this._timecodedGroup!.getRelativePointerPosition()
      })
    })

    this._timecodedEventCatcher.on('mouseenter', (event) => {
    ...
    this._timecodedEventCatcher.on('mouseout', (event) => {
    ...
    this._timecodedEventCatcher.on('mouseleave', (event) => {
    ...
    this._timecodedEventCatcher.on('click', (event) => {
      this.onClick$.next({
        evt: event.evt,
        position: this._timecodedGroup!.getRelativePointerPosition()
      })
    ...
    // timecode display
    this._timecodeDisplay = new TimecodeDisplay({
      style: {
        fontSize: this.style.descriptionTextFontSize,
        fill: this.style.descriptionTextFill
      }
    }, this._timeline!, this._videoController!);


    let timecodeDisplayFlexItem = new KonvaFlexItem({
      width: '100%',
      height: '100%',
    }, new KonvaComponentFlexContentNode(this._timecodeDisplay))
  ...
  override onStyleChange() {
    ...
    if (this._timecodeDisplay) {
      this._timecodeDisplay.style = {
        ...this._timecodeDisplay.style,
        fontSize: this.style.descriptionTextFontSize,
        fill: this.style.descriptionTextFill
      }
    ...
    for (let i = 0; i < (this._tickTotalDivisions + 1); i++) {
      ...
      if (strategy === 'move') {
        ...
        let text = new Konva.Text({
          ...
          text: `${this._timeline.timelinePositionToTimeFormatted(tickGroupX)}`,
          listening: false
        })
