...
export interface BarChartLaneConfig extends TimelineLaneConfig<BarChartLaneStyle> {
  vttUrl?: string;
...
export class BarChartLane extends BaseTimelineLane<BarChartLaneConfig, BarChartLaneStyle> implements BarChartLaneApi {
  private readonly _itemsMap: Map<number, BarChartLaneItem> = new Map<number, BarChartLaneItem>();

  protected readonly _onSettleLayout$: Subject<void> = new Subject<void>();

  private _vttUrl?: string;
  private _vttFile?: BarChartVttFile;
  private _valueTransformFn: (value: number) => number;
  private _itemProcessFn?: (item: BarChartLaneItem, index: number) => void;
  ...
  private fetchVttFile(vttUrl: string, axiosConfig?: AxiosRequestConfig): Observable<BarChartVttFile | undefined> {
    return BarChartVttFile.create(vttUrl, axiosConfig).pipe(map(vttFile => {
  ...
  private createFromVttFile(vttFile: BarChartVttFile) {
    this.clearItems();
