...
import {VttFileParsed} from '../track';
...
export class VttUtil {
  ...
  static concatSegmented(vttTexts: string[]): string | undefined {
    if (vttTexts && vttTexts.length > 1) {
      let first: VttFileParsed = webvtt.parse(vttTexts[0], {strict: true, meta: true});
  ...
  static createWebvttBlob(webvttText: string): string {
    return FileUtil.createBlob([webvttText], 'text/vtt');
  }
